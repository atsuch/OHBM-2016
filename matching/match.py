...
from .match_plotting import (plot_matched_components, plot_comparison_matrix,
...
def do_match_analysis(dataset, ica_images, key="wb", n_components=20,
                      scoring='correlation', nii_dir=None,
                      plot=True, plot_dir=None,):
...
def match_main(dataset, key="wb", n_components=20, plot=True,
    ...
    return do_match_analysis(
        dataset=dataset, images=imgs, key=key, n_components=n_components,
        plot=plot, scoring=scoring, force=force, nii_dir=nii_dir,
        plot_dir=plot_dir)
